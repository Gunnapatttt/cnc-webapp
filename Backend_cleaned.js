...
/**
 * Helper function to write debug information to Debug sheet
 * Eliminates code duplication across the application
 */
function writeDebugLog(...args) {
  try {
    var debugSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(DEBUG_SHEET_NAME);
    if (!debugSheet) {
      debugSheet = SpreadsheetApp.getActiveSpreadsheet().insertSheet(DEBUG_SHEET_NAME);
    }
    debugSheet.appendRow([new Date(), ...args]);
  } catch (e) {
...
function getCNCLogSheet() {
  ...
  if (!sheet) {
    writeDebugLog('Early return: Sheet not found');
    console.error(`Sheet '${SHEET_NAME}' not found`);
    writeDebugLog('RETURN/THROW before OT block: Sheet not found');
  ...
  try {
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (rowStatus === 'OPEN' &&
        ...
        // Log the duplicate found for debugging
        writeDebugLog(
          'DUPLICATE JOB FOUND', 
          'Row: ' + (i + 1),
    ...
    writeDebugLog('ERROR in isDuplicateOpenJob', e.toString());
  }
  return false;
}
...
function submitLog(data) {
  ...
  writeDebugLog('submitLog called', JSON.stringify(data));
  writeDebugLog('data.action value', String(data.action));
  writeDebugLog('Before START_OT if', 'data.action: ' + String(data.action));
  writeDebugLog(
    'data.action details',
  ...
  writeDebugLog('RIGHT BEFORE IF START_OT');
  ...
  if (
    ...
    if (String(data.action).toUpperCase().replace(/\s+/g, '') === 'START_OT') {
      writeDebugLog('AFTER IF START_OT');
      writeDebugLog('Entering START_OT loop', 'values.length: ' + values.length);
