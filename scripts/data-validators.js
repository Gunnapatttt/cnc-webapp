...
/**
 * Validates step number from string value (for form data validation)
 * @param {string} stepNo - Step number string to validate
 * @returns {boolean} - True if valid, false otherwise
 */
function validateStepNoString(stepNo) {
    if (!stepNo || typeof stepNo !== 'string') return false;
    // Remove any non-digit and non-plus characters
    let value = stepNo.replace(/[^0-9+]/g, '');
    // Remove leading and trailing plus signs
    value = value.replace(/^\+/, '').replace(/\+$/, '');
    // Replace multiple consecutive plus signs with single plus
    value = value.replace(/\+\+/g, '+');
...
function validateFormData(formData) {
    ...
    if (!validateStepNoString(formData.stepNo)) {
